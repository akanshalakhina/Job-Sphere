...
export const FloatingChatbot = () => {
  ...
  const handleSubmit = (e) => {
    ...
    sendChatbotMessage(inputText.trim());
    setInputText('');
    
    // Simulate AI thinking typing dots
    setIsTyping(true);
    setTimeout(() => {
      setIsTyping(false);
    }, 800);
    ...
    <div className="fixed bottom-6 right-6 z-[95] pointer-events-none">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div className="p-4 border-b border-slate-100 dark:border-navy-800/80 bg-gradient-to-r from-brand-600 to-indigo-650 flex items-center justify-between text-white">
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 no-scrollbar">
                  ...
                  <div
                    className={`max-w-[80%] p-3.5 rounded-2xl text-xs leading-relaxed ${
                      msg.sender === 'user'
                        ...
                        : 'bg-slate-100 dark:bg-navy-900 text-slate-800 dark:text-slate-200 border border-slate-200/40 dark:border-navy-850 rounded-tl-none'
                    }`}
                  >
                    {msg.text}
                  </div>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="p-3.5 rounded-2xl bg-slate-100 dark:bg-navy-900 text-slate-500 border border-slate-250/20 dark:border-navy-850 rounded-tl-none flex items-center gap-1">
            ...
            <div className="px-4 py-2 border-t border-slate-100 dark:border-navy-900 flex gap-2 overflow-x-auto no-scrollbar scroll-smooth flex-shrink-0 bg-slate-50/50 dark:bg-navy-950/40">
              {quickPills.map(pill => (
                <button
                  ...
                  className="text-[10px] font-semibold px-2.5 py-1.5 rounded-full border border-slate-200 dark:border-navy-800 bg-white dark:bg-navy-900 text-slate-600 dark:text-slate-350 hover:border-brand-500 dark:hover:border-brand-400 hover:text-brand-500 transition-all whitespace-nowrap flex-shrink-0"
                >
                  {pill}
                </button>
              ))}
      ...
      <button
        ...
        className="pointer-events-auto relative p-4 rounded-full bg-gradient-to-tr from-brand-600 to-indigo-650 hover:scale-105 active:scale-95 transition-all text-white flex items-center justify-center shadow-xl shadow-brand-500/20 group hover:rotate-6"
      >
