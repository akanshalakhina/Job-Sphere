...
export const RecruiterDashboard = () => {
  const { 
    ...
    scheduleInterview, addPendingJob, verifiedCompanies 
  } = useAppState();
  ...
  const [companySize, setCompanySize] = useState('10,000+ employees');
  const [companyWebsite, setCompanyWebsite] = useState('stripe.com');
  const [companyDesc, setCompanyDesc] = useState('Stripe is a financial infrastructure platform for the internet.');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex flex-col gap-8 z-10 relative">
            ...
            <button
              ...
              className={`flex-1 lg:flex-initial px-4 py-2.5 text-xs font-bold rounded-xl transition-all ${
                activeTab === tab.id
                  ...
                  : 'text-slate-600 dark:text-slate-455 hover:bg-slate-50 dark:hover:bg-navy-800/40'
              }`}
            >
              {tab.label}
            </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        {[
          ...
          { label: "Offers Extended", value: offeredCount, icon: CheckCircle, color: "bg-emerald-500/10 text-emerald-555" },
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 overflow-x-auto pb-4 items-start select-none">
            {columns.map(colName => {
              ...
              return (
                <div
                  key={colName}
                  className="flex-1 min-w-[220px] border border-slate-200/60 dark:border-navy-850 bg-slate-100/50 dark:bg-navy-900/30 rounded-2xl p-4 flex flex-col gap-4 text-left"
                >
                  <div className="flex items-center justify-between border-b border-slate-200/55 dark:border-navy-855 pb-2">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[9px] text-slate-450 mt-0.5 truncate">{cand.title}</p>
                          </div>
                        </div>

                        <p className="text-[8.5px] text-slate-400">Position: <span className="text-slate-650 dark:text-slate-350 font-bold truncate">{cand.appliedFor}</span></p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start text-left">
          ...
          <div className="lg:col-span-2 border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm overflow-x-auto">
            <div className="flex justify-between items-center mb-4">
              <h4 className="text-xs uppercase font-extrabold text-slate-400 tracking-wider">Shortlisted Candidates Pool</h4>
              {compareIds.length > 0 && (
            ...
            <table className="w-full text-left border-collapse min-w-[500px]">
              <thead>
                <tr className="border-b border-slate-100 dark:border-navy-800 pb-3">
                  <th className="text-[10px] uppercase font-bold text-slate-450 tracking-wider p-3">Compare</th>
          ...
          {compareIds.length === 2 && (
            <div className="lg:col-span-3 border border-indigo-200 dark:border-navy-800/80 bg-gradient-to-tr from-brand-500/5 to-indigo-500/5 dark:from-brand-500/10 dark:to-indigo-500/10 rounded-3xl p-6 shadow-lg flex flex-col gap-6 w-full">
              <div className="flex justify-between items-center">
                <div>
                  <h4 className="text-sm font-extrabold text-slate-850 dark:text-white flex items-center gap-1.5">
                    <Sparkles className="w-4 h-4 text-brand-500 animate-pulse" />
                    AI Applicant Comparison Sourcing
                  </h4>
                  <p className="text-[10px] text-slate-400 mt-0.5">Dual-profile analytics match review</p>
                </div>
                <button 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-200 dark:border-navy-850">
                {compareIds.map((candId, idx) => {
                  const candidate = candidates.find(c => c.id === candId);
                  if (!candidate) return null;
                  return (
                    <div key={candidate.id} className="p-4 rounded-2xl border border-slate-200 dark:border-navy-850 bg-white/70 dark:bg-navy-950/40 flex flex-col gap-4">
                      <div className="flex items-center gap-3">
                        <img src={candidate.avatar} alt={candidate.name} className="w-10 h-10 rounded-full object-cover border-2 border-brand-500" />
                        <div>
                          <span className="text-[9px] font-bold text-slate-400 uppercase">Applicant #{idx + 1}</span>
                          <h5 className="text-xs font-bold text-slate-850 dark:text-white mt-0.5 leading-none">{candidate.name}</h5>
                      ...
                      <div className="p-3.5 rounded-2xl bg-indigo-500/5 border border-indigo-500/10 text-[9.5px] leading-relaxed text-slate-655 dark:text-slate-350">
                        <span className="font-bold flex items-center gap-1 mb-1 text-indigo-650 dark:text-indigo-400">📝 Sourcing Review</span>
          ...
          <form onSubmit={handleScheduleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                className="p-3 bg-slate-50/50 dark:bg-navy-950/40 border border-slate-200 dark:border-navy-800/50 rounded-xl outline-none text-xs text-slate-750 dark:text-slate-200 focus:border-brand-500"
                required
              >
      ...
      {activeTab === 'postjob' && (
        <div className="max-w-xl mx-auto w-full border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 backdrop-blur-xl rounded-3xl p-6 md:p-8 shadow-sm flex flex-col gap-6 text-left">
          <div>
            ...
            <p className="text-[10px] text-slate-450 dark:text-slate-400 mt-0.5 flex items-center gap-1 text-amber-500">
          ...
          <form onSubmit={handlePostJobSubmit} className="flex flex-col gap-4">
            ...
            <button
              type="submit"
              className="w-full py-3.5 rounded-2xl bg-brand-650 hover:bg-brand-700 text-white font-bold text-xs shadow-md shadow-brand-500/10 hover:scale-[1.01] active:scale-[0.99] transition-all mt-2"
            >
      ...
      {activeTab === 'branding' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start text-left">
          ...
          <div className="lg:col-span-7 border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-6 shadow-sm flex flex-col gap-6">
            <div>
              <h4 className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-1.5">
                <Edit3 className="w-4.5 h-4.5 text-brand-500" />
            ...
            <div className="border border-slate-200/80 dark:border-navy-800 bg-white dark:bg-navy-950 p-6 shadow-2xl rounded-3xl flex flex-col gap-6 relative overflow-hidden">
              ...
              <div className="flex justify-between items-start border-b border-slate-100 dark:border-navy-900 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-extrabold text-slate-850 dark:text-white leading-tight">{companyName}</h3>
                ...
                {isCompanyVerified ? (
                  ...
                  <span className="bg-slate-200 dark:bg-navy-900 text-slate-500 text-[9.5px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                    <AlertCircle className="w-3.5 h-3.5 text-slate-450" /> Pending Verification
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex flex-col gap-0.5">
                  ...
                  <p className="text-[11px] text-slate-550 dark:text-slate-350 leading-relaxed font-sans">{companyDesc}</p>
                </div>
                <div className="flex flex-col gap-0.5 mt-2">
                  <span className="text-[9px] uppercase font-bold text-slate-400">Staff Benefits</span>
                  <p className="text-[11px] text-slate-555 dark:text-slate-350 leading-relaxed font-sans">{companyBenefits}</p>
                </div>
              </div>
            </div>
          </div>
