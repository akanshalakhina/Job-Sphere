  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex flex-col gap-16 z-10 relative">
      ...
      <section className="text-center flex flex-col items-center gap-4 pt-4">
        ...
        <h2 className="text-2xl sm:text-4xl font-extrabold text-slate-850 dark:text-white leading-tight">Empowering Global Sourcing</h2>
        ...
        {values.map((val, i) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <h4 className="text-xs font-bold text-slate-850 dark:text-white mb-1.5">{val.title}</h4>
                <p className="text-[11px] text-slate-500 dark:text-slate-450 leading-relaxed">{val.desc}</p>
              </div>
            </div>
          );
        })}
      ...
      <section className="flex flex-col gap-10">
        <h3 className="text-xl font-extrabold text-slate-850 dark:text-white text-center">Company Milestones</h3>
        <div className="flex flex-col gap-8 max-w-xl mx-auto w-full relative pl-6 border-l border-slate-200 dark:border-navy-800">
          {milestones.map((ms, idx) => (
            <div key={idx} className="relative flex flex-col gap-1">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-450 leading-relaxed mt-0.5">{ms.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="flex flex-col gap-10">
        <h3 className="text-xl font-extrabold text-slate-850 dark:text-white text-center">Executive Team</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {team.map((user, i) => (
            ...
            >
              <img src={user.avatar} alt={user.name} className="w-20 h-20 rounded-full object-cover mb-4 border border-slate-200/50 group-hover:scale-105 transition-transform duration-300 shadow-md" />
              <h4 className="text-xs font-bold text-slate-800 dark:text-white">{user.name}</h4>
              <p className="text-[9.5px] text-slate-450 mt-0.5">{user.role}</p>
            </div>
          ))}
        </div>
      </section>
