        ...
        <div className="flex-1 flex flex-col gap-6 w-full">
          ...
          <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 backdrop-blur-xl rounded-3xl p-6 md:p-8 shadow-sm flex flex-col md:flex-row items-start justify-between gap-6">
            <div className="flex gap-4">
              <div className="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-navy-950 flex items-center justify-center font-extrabold text-2xl text-brand-500 border border-slate-200/40 dark:border-navy-850 flex-shrink-0">
                {job.logo}
              </div>
              <div className="flex flex-col gap-1.5">
                <h2 className="text-xl sm:text-2xl font-extrabold text-slate-850 dark:text-white leading-tight">{job.title}</h2>
                <p className="text-xs text-slate-600 dark:text-slate-350 font-bold flex items-center gap-2">
                  <span>{job.company}</span>
                  <span className="text-slate-300">•</span>
                  <span className="font-semibold text-slate-550 flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-slate-400" /> {job.location}</span>
                </p>
                <div className="flex flex-wrap gap-2 mt-2">
                  <span className="text-[9px] font-bold px-2.5 py-1 rounded-full bg-slate-100 dark:bg-navy-950 text-slate-550 dark:text-slate-400">{job.experience}</span>
                  <span className="text-[9px] font-bold px-2.5 py-1 rounded-full bg-slate-100 dark:bg-navy-950 text-slate-550 dark:text-slate-400">{job.department}</span>
                  <span className="text-[9px] font-bold px-2.5 py-1 rounded-full bg-brand-500/10 text-brand-650 dark:text-brand-400">{job.postedTime}</span>
                </div>
              </div>
            </div>
          ...
          <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-6 md:p-8 shadow-sm flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-slate-550 dark:text-slate-350 leading-relaxed">{job.description}</p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <ul className="flex flex-col gap-2.5">
                {job.responsibilities.map((resp, i) => (
                  <li key={i} className="flex gap-2.5 text-xs text-slate-650 dark:text-slate-350 leading-relaxed items-start">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                {job.skills.map(s => (
                  <span
                    key={s}
                    className="text-xs font-bold px-3 py-1 rounded-xl bg-slate-150/50 dark:bg-navy-950/50 text-slate-600 dark:text-slate-350 border border-slate-200/25 dark:border-navy-800"
                  >
                    {s}
                  </span>
                ))}
          ...
          <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-6 shadow-sm flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                <img src={job.recruiter.avatar} alt={job.recruiter.name} className="w-12 h-12 rounded-full object-cover" />
                <div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => addToast(`Messaged ${job.recruiter.name}!`, 'success')}
                  className="p-2.5 rounded-xl border border-slate-250 dark:border-navy-800 hover:bg-slate-100 dark:hover:bg-navy-800/40 text-slate-500 dark:text-slate-350 transition-all flex items-center gap-1.5 text-[10px] font-semibold"
                >
        ...
        <aside className="w-full lg:w-80 flex flex-col gap-6 flex-shrink-0 lg:sticky lg:top-24">
          ...
          <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm flex flex-col items-center text-center gap-4">
            ...
            <div>
              ...
              <p className="text-[9.5px] text-slate-450 dark:text-slate-400 mt-1 leading-relaxed">
          ...
          <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm flex flex-col gap-4">
            <h4 className="text-xs font-bold text-slate-850 dark:text-white uppercase tracking-wider">Position Overview</h4>
            
            <div className="flex flex-col gap-3">
              <div className="flex justify-between text-xs font-semibold">
                <span className="text-slate-400">Compensation:</span>
                <span className="text-slate-800 dark:text-slate-200 flex items-center gap-0.5"><DollarSign className="w-3.5 h-3.5 text-slate-450" /> {job.salary}</span>
              </div>
              <div className="flex justify-between text-xs font-semibold">
          ...
          <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm flex flex-col gap-4">
            <h4 className="text-xs font-bold text-slate-850 dark:text-white uppercase tracking-wider">Alternative Jobs</h4>
            <div className="flex flex-col gap-3">
              {similarJobs.map(sj => (
                <Link
                  key={sj.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-slate-450 truncate">{sj.company} • {sj.location}</p>
                  </div>
                </Link>
              ))}
            </div>
