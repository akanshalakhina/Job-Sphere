      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
                ...
                <button
                  ...
                  className={`text-xs font-medium px-3.5 py-2 rounded-xl text-left transition-colors ${
                    selectedDept === dept
                      ...
                      : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-navy-800/40'
                  }`}
                >
                  {dept}
                </button>
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="flex justify-between items-center px-1">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold">Showing <span className="text-slate-800 dark:text-white font-bold">{filteredJobs.length}</span> corresponding positions</p>
          </div>
          ...
          {filteredJobs.length > 0 ? (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-navy-950 flex items-center justify-center font-extrabold text-lg text-brand-500 border border-slate-200/40 dark:border-navy-800 flex-shrink-0">
                        {job.logo}
                      </div>
                      <div className="flex flex-col gap-1">
                        <h4 className="text-sm font-bold text-slate-800 dark:text-white group-hover:text-brand-500 transition-colors flex items-center gap-2">
                          {job.title}
                        </h4>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">
                          <span className="font-bold text-slate-700 dark:text-slate-300">{job.company}</span> • <span className="flex-shrink-0">{job.location}</span>
                        </p>
            ...
            <div className="border border-slate-200/50 dark:border-navy-800 bg-white/40 dark:bg-navy-900/20 rounded-3xl p-12 text-center flex flex-col items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 max-w-[280px] mx-auto mt-0.5">We couldn't find any job matches. Try adjusting your sidebar filters or changing the search keywords.</p>
              </div>
              <button
                onClick={() => { setSearch(''); setSelectedDept('All'); setSelectedWorkplace('All'); setSelectedExp('All'); }}
                className="px-5 py-2.5 rounded-xl border border-slate-200 dark:border-navy-800 bg-white dark:bg-navy-900 text-xs font-bold text-slate-700 dark:text-slate-200"
              >
                Reset Filters
              </button>
            </div>
