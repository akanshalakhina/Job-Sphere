...
export const ContactPage = () => {
  ...
  const { submitContactMessage } = useAppState();
  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  const [msg, setMsg] = useState('');
  const [companyName, setCompanyName] = useState('');
  const [sourcingNeeds, setSourcingNeeds] = useState('SaaS Gold Sourcing Plan');
  const [isInquiry, setIsInquiry] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name || !email || !msg || (isInquiry && !companyName)) {
    ...
    const result = await submitContactMessage({
      ...
      email,
      message: msg,
      type: isInquiry ? 'business' : 'support',
      companyName: isInquiry ? companyName : '',
      sourcingNeeds: isInquiry ? sourcingNeeds : '',
    });
    setIsSubmitting(false);

    if (!result.success) {
      addToast(result.error || 'Unable to submit your message right now.', 'error');
      return;
    }

    addToast(
      result.emailSent
        ? 'Message submitted and confirmation email sent!'
        : 'Message submitted. Email delivery is disabled in this environment.',
      'success',
    );
    setName('');
    setEmail('');
    setMsg('');
    setCompanyName('');
    setSourcingNeeds('SaaS Gold Sourcing Plan');
  };
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex flex-col gap-12 z-10 relative">
      ...
      <div className="text-center flex flex-col items-center gap-2">
        ...
        <h2 className="text-2xl sm:text-4xl font-extrabold text-slate-800 dark:text-white leading-tight">Get in touch</h2>
        <p className="text-xs text-slate-600 dark:text-slate-400 max-w-lg mt-0.5 leading-relaxed">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        ...
        <div className="flex flex-col gap-6">
          <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 backdrop-blur-xl rounded-3xl p-6 shadow-sm flex flex-col gap-5">
            <h4 className="text-sm font-bold text-slate-800 dark:text-white mb-2">Direct Contact Directories</h4>
                ...
                return (
                  <div key={i} className="flex gap-3.5 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-700 dark:text-slate-200">{info.val}</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="border border-slate-200/60 dark:border-navy-800/40 bg-gradient-to-br from-brand-600 to-indigo-600 text-white rounded-3xl p-6 shadow-md relative overflow-hidden">
        ...
        <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 backdrop-blur-xl rounded-3xl p-6 shadow-sm text-left">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <button
              ...
              className="w-full py-3.5 rounded-2xl border border-brand-200 bg-brand-50 hover:bg-brand-100 text-brand-700 dark:border-brand-600 dark:bg-brand-600 dark:hover:bg-brand-700 dark:text-white font-bold text-xs shadow-md shadow-brand-500/10 hover:scale-[1.01] active:scale-[0.99] transition-all flex items-center justify-center gap-1.5 disabled:opacity-60 disabled:cursor-not-allowed"
            >
              <Send className="w-3.5 h-3.5" />
              {isSubmitting ? 'Submitting...' : isInquiry ? 'Submit Business Inquiry' : 'Submit Support Ticket'}
            </button>
          </form>
        </div>
