...
import { 
  ...
  Trash2, Mail, Phone, Link2, Plus, FileText, AlertCircle, PlusCircle, HelpCircle, MessageSquareCode
} from 'lucide-react';
...
export const CandidateDashboard = () => {
  ...
  const { 
    appliedJobIds = [], savedJobIds = [], jobs = [], interviews = [], 
  ...
  // Resume Builder local states synced with global context & currentUser profile properties
  const [personal, setPersonal] = useState(() => ({
    name: currentUser.name || resumeBuilderData?.personal?.name || '',
    email: currentUser.email || resumeBuilderData?.personal?.email || '',
    phone: resumeBuilderData?.personal?.phone || '',
    linkedin: resumeBuilderData?.personal?.linkedin || '',
    github: resumeBuilderData?.personal?.github || '',
    portfolio: resumeBuilderData?.personal?.portfolio || ''
  }));
  const [education, setEducation] = useState(() => ({
    degree: currentUser.degree || resumeBuilderData?.education?.degree || '',
    college: currentUser.college || resumeBuilderData?.education?.college || '',
    cgpa: resumeBuilderData?.education?.cgpa || '',
    gradYear: currentUser.graduationYear || resumeBuilderData?.education?.gradYear || ''
  }));
  const [techSkills, setTechSkills] = useState(() => (
    (currentUser.skills && currentUser.skills.length > 0) 
      ? currentUser.skills.join(', ') 
      : (resumeBuilderData?.skills?.technical?.join(', ') || '')
  ));
  ...
  React.useEffect(() => {
    if (currentUser) {
      ...
      setEducation(prev => ({
        ...
        gradYear: currentUser.graduationYear || prev.gradYear,
      }));
      if (currentUser.skills && currentUser.skills.length > 0) {
        setTechSkills(currentUser.skills.join(', '));
      }
    }
  }, [currentUser]);
  ...
  // Application activity chart data from current app state.
  const chartData = [
    { name: 'Applied', applications: appliedJobIds.length },
    { name: 'Saved', applications: savedJobIds.length },
    { name: 'Interviews', applications: interviews.length },
  ];

  const resumeDetails = currentUser.resumeDetails;
  const hasResumeScore = Number.isFinite(resumeDetails?.atsScore);
  const atsScore = hasResumeScore ? resumeDetails.atsScore : 0;
  const activeInterview = interviews[0] || null;
  const trackerRounds = activeInterview
    ? (
        activeInterview.rounds?.length
          ? activeInterview.rounds
          : [{
              name: activeInterview.type || 'Interview Round',
              description: `${activeInterview.jobTitle || activeInterview.role || 'Interview'} scheduled with ${activeInterview.company || 'the recruiter'}.`,
              status: activeInterview.status === 'completed' ? 'completed' : 'active',
              coachPrompt: 'Practice your strongest project story and role-specific fundamentals.',
            }]
      )
    : [];
  ...
  const roundStatusLabel = (status) => {
    ...
    if (status === 'active' || status === 'scheduled') return 'Active Round';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex flex-col gap-8 z-10 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="text-left">
          <h2 className="text-2xl font-extrabold text-slate-800 dark:text-white">Student Command Center</h2>
        ...
        {/* AI Coach Button */}
        <button
          data-testid="practice-ai-coach-btn"
          onClick={() => setCoachModalOpen(true)}
          className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-brand-600 to-indigo-600 text-white text-xs font-bold shadow-md shadow-brand-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2"
        >
      ...
      {/* Tab Selection */}
      <div className="border border-slate-200 dark:border-navy-800 bg-white dark:bg-navy-900 rounded-2xl p-1 flex flex-wrap lg:flex-nowrap gap-1 w-full">
        {[
          ...
          <button
            ...
            className={`flex-1 md:flex-initial px-4 py-2.5 text-xs font-bold rounded-xl transition-all whitespace-nowrap ${
              activeTab === tab.id
      ...
      <AnimatePresence mode="wait">
          ...
          >
            ...
            <aside className="w-full lg:w-64 flex flex-col gap-6 flex-shrink-0">
              ...
              <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 backdrop-blur-xl rounded-3xl p-5 shadow-sm flex flex-col items-center text-center">
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 leading-snug">{currentUser.title}</p>
                {currentUser.college && (
                  <p className="text-[10px] text-brand-500 font-bold mt-1">{currentUser.college}</p>
                )}
                
                <div className="h-px bg-slate-100 dark:bg-navy-800 w-full my-4" />
                
                <div className="flex flex-col gap-2 w-full text-left">
                  <div className="flex justify-between items-center text-[10.5px] font-bold text-slate-500 uppercase tracking-wider">
                      ...
                      <div 
                        ...
                        className={`flex flex-col items-center justify-center p-2 rounded-xl border text-center transition-all ${
                          badge.unlocked 
                            ...
                            : 'border-slate-200 dark:border-navy-800 opacity-40 filter grayscale'
                        }`}
                        title={badge.name}
                      >
            ...
            <div className="flex-1 flex flex-col gap-6 w-full text-left">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Active Submissions</p>
                  </div>
                </div>
                ...
                <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Scheduled Rounds</p>
                  </div>
                </div>
                ...
                <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xl font-extrabold text-slate-800 dark:text-white">{hasResumeScore ? `${atsScore}%` : 'Not analyzed'}</p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold">AI ATS Metric Score</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm flex flex-col gap-4">
                  <div>
                    <h4 className="text-sm font-bold text-slate-800 dark:text-white">Application Activity</h4>
                    <p className="text-[10px] text-slate-400">Current applications, saves, and interview rounds from app state</p>
                  </div>
                ...
                <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm flex flex-col justify-between gap-4">
                  ...
                  <div className="flex flex-col gap-3 flex-grow overflow-y-auto max-h-[160px] no-scrollbar">
                    {interviews.map(int => (
                      <div key={int.id} className="p-3 rounded-xl border border-slate-100 dark:border-navy-800 bg-slate-50/50 dark:bg-navy-950/40 text-[10.5px]">
                        <div className="flex justify-between items-start font-bold text-slate-800 dark:text-white">
                          <span>{int.jobTitle || int.role}</span>
                          <span className="text-[9px] text-brand-500 bg-brand-500/10 px-1.5 py-0.5 rounded">{int.status}</span>
                        </div>
                        <p className="text-slate-500 mt-1">Candidate: <span className="font-bold text-slate-700 dark:text-slate-300">{int.candidateName}</span></p>
                  ...
                  <button 
                    ...
                    className="w-full py-2 rounded-xl bg-slate-100 dark:bg-navy-800 hover:bg-slate-200 text-[10px] font-bold text-center text-slate-700 dark:text-slate-300"
                  >
              ...
              <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  <h4 className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-1.5">
                    ...
                    <span className="text-[9px] font-bold px-2.5 py-0.5 rounded bg-brand-500/10 text-brand-500">{recommendedJobs.length} live matches</span>
                  </h4>
                ...
                <div className="flex flex-col gap-3">
                  {recommendedJobs.length > 0 ? recommendedJobs.map(job => (
                    <div
                      ...
                      className="border border-slate-100 dark:border-navy-800 bg-white/50 dark:bg-navy-950/40 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-brand-500/30 dark:hover:border-brand-400/30 hover:scale-[1.005] cursor-pointer transition-all duration-200"
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-navy-800 flex items-center justify-center font-bold text-sm text-brand-500">{job.logo}</div>
                        <div>
                    ...
                    <div className="rounded-2xl border border-dashed border-slate-200 dark:border-navy-800 bg-slate-50/50 dark:bg-navy-950/30 p-5 text-xs font-semibold text-slate-500 dark:text-slate-400 text-center">
                      Approved jobs with strong skill overlap will appear here.
                    </div>
                  )}
                </div>
              </div>
              ...
              <form onSubmit={handleSyncBuilder} className="flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {experience.map((exp, i) => (
                    <div key={i} className="p-4 rounded-2xl border border-slate-100 dark:border-navy-800 bg-slate-50/20 dark:bg-navy-950/20 flex flex-col gap-3">
            ...
            <div className="lg:col-span-5 flex flex-col gap-4">
              <div className="flex justify-between items-center">
                ...
                <button 
                  ...
                  className="px-3.5 py-1.5 rounded-xl border border-slate-200 dark:border-navy-800 bg-white dark:bg-navy-900 hover:bg-slate-100 text-[10px] font-bold text-slate-700 dark:text-slate-300 shadow flex items-center gap-1"
                >
              ...
              <div id="resume-sheet" className="w-full border border-slate-200 dark:border-navy-800 bg-white text-slate-900 p-6 md:p-8 shadow-2xl rounded-2xl flex flex-col gap-6 text-left relative font-serif aspect-[1/1.4] overflow-y-auto no-scrollbar select-text">
                  ...
                  <div className="flex flex-col gap-4">
                    {experience.map((exp, idx) => (
                      <div key={idx} className="flex flex-col text-[10.5px]">
                        ...
                        <p className="text-[9.5px] italic text-brand-600 font-sans font-bold">{exp.company}</p>
                        <p className="text-[9.5px] text-slate-600 mt-1 leading-relaxed font-sans">{exp.desc}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          >
            ...
            <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-6 shadow-sm flex flex-col justify-between items-center text-center gap-6">
              ...
              <div className="relative w-36 h-36 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90">
                  <circle cx="72" cy="72" r="62" stroke="currentColor" className="text-slate-100 dark:text-navy-800" strokeWidth="8" fill="transparent" />
                ...
                <div className="absolute text-center">
                  <span className="text-3xl font-black text-slate-800 dark:text-white">{hasResumeScore ? `${atsScore}%` : '--'}</span>
              ...
              <div className="flex flex-col gap-3 w-full text-left mt-2">
                {hasResumeScore ? (
                  [
                    ...
                    { name: "Industry Relevancy Matrix", score: Math.max(0, Math.min(atsScore - 6, 92)), color: "bg-indigo-500" },
                    { name: "Formatting Alignment Check", score: Math.min(atsScore + 2, 95), color: "bg-cyan-500" }
                  ].map((crit, idx) => (
                  ...
                  <div className="rounded-2xl border border-dashed border-slate-200 dark:border-navy-800 bg-slate-50/60 dark:bg-navy-950/30 p-4 text-[10.5px] font-semibold text-slate-500 dark:text-slate-400">
                    Upload or analyze a resume to populate this scorecard.
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                  {[
                    ...
                    >
                      <div>
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-bold text-slate-800 dark:text-white">{kw.word}</span>
              ...
              <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-6 shadow-sm flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  <div>
                    <h4 className="text-sm font-bold text-slate-800 dark:text-white">Suggested Certifications</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-1">
                  {CERTIFICATIONS.slice(0, 2).map(cert => (
                    <div key={cert.id} className="p-4 rounded-2xl border border-slate-100 dark:border-navy-800 bg-slate-50/30 dark:bg-navy-900/30 flex justify-between items-center gap-4">
                      <div>
                        ...
                        <h5 className="text-xs font-bold text-slate-800 dark:text-white mt-1.5 leading-tight">{cert.name}</h5>
        ...
        {activeTab === 'upskill' && (
          ...
          >
            ...
            <div className="lg:col-span-8 border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-6 shadow-sm flex flex-col gap-6">
              ...
              {/* Track Selector */}
              <div className="flex justify-between items-center border-b border-slate-100 dark:border-navy-800 pb-4">
                <div>
                  <h4 className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-1.5">
              ...
              <div className="flex flex-col gap-6">
                <div>
                  <h3 className="text-xs font-bold text-slate-800 dark:text-white uppercase tracking-wider">{ROADMAPS[selectedTrack].title}</h3>
                  <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">{ROADMAPS[selectedTrack].description}</p>
                </div>

                <div className="relative pl-6 border-l-2 border-slate-100 dark:border-navy-800 flex flex-col gap-6 ml-2 select-none">
                  {ROADMAPS[selectedTrack].steps.map((step, idx) => (
                    <div key={step.id} className="relative">
                      ...
                      <div>
                        <div className="flex justify-between items-center">
                          <h5 className={`text-xs font-bold ${step.completed ? 'text-slate-800 dark:text-slate-200' : 'text-slate-500 dark:text-slate-500'}`}>{step.title}</h5>
                          <span className={`text-[8.5px] font-bold px-2 py-0.5 rounded ${
                            step.completed ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-100 dark:bg-navy-800 text-slate-400'
              ...
              <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-5 shadow-sm flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-3">
                  {TRENDING_SKILLS.slice(0, 4).map(skill => (
                    <div key={skill.id} className="flex justify-between items-center border-b border-slate-100 dark:border-navy-800/50 pb-2.5">
                      <div>
                      ...
                      <span className="text-[9px] font-extrabold uppercase text-slate-500 bg-slate-100 dark:bg-navy-800 px-2 py-0.5 rounded">{skill.demand}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {activeTab === 'interviews' && (
          ...
          >
            ...
            {activeInterview && (
              <div className="rounded-2xl border border-slate-200 dark:border-navy-800 bg-slate-50/50 dark:bg-navy-950/30 p-4 text-xs text-slate-600 dark:text-slate-300">
                Tracking <span className="font-bold text-slate-800 dark:text-white">{activeInterview.jobTitle || activeInterview.role}</span>
                {activeInterview.company ? ` at ${activeInterview.company}` : ''} on {activeInterview.date} at {activeInterview.time}.
                <span className="ml-2 font-bold text-brand-500">
                  Calendar: {activeInterview.calendar?.status || 'skipped'}
                </span>
              </div>
            )}
            ...
            {trackerRounds.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pb-4">
                {trackerRounds.map((round, idx) => {
                  const isActive = round.status === 'active' || round.status === 'scheduled';
                  const isCompleted = round.status === 'completed';
                  return (
                    <div
                      key={round._id || round.name || idx}
                      className={`border rounded-2xl p-4 flex flex-col justify-between gap-4 transition-all relative overflow-hidden ${
                        isActive
                          ...
                          : isCompleted
                          ? 'border-emerald-200 bg-emerald-500/5'
                          : 'border-slate-200 dark:border-navy-800 opacity-55'
                      }`}
                    >
                      <div className="flex justify-between items-start">
                      ...
                      <div>
                        <h5 className="text-xs font-bold text-slate-800 dark:text-white leading-tight">{round.name}</h5>
                        <p className="text-[10px] text-slate-500 mt-1 leading-snug">{round.description}</p>
                      </div>

                      {isActive && (
                        <div className="p-3.5 rounded-xl bg-brand-500/10 border border-brand-500/10 flex flex-col gap-2 mt-1">
                          <p className="text-[9px] font-extrabold uppercase text-slate-700 dark:text-slate-300">AI Coach Question Tips</p>
                          <p className="text-[10px] text-brand-600 dark:text-brand-400 italic leading-snug">"{round.coachPrompt || 'Practice your role-specific fundamentals and project storytelling.'}"</p>
                          <button
                            data-testid="practice-ai-coach-btn"
                            onClick={() => setCoachModalOpen(true)}
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 dark:border-navy-800 p-8 text-center text-xs text-slate-400">
                No interview rounds are scheduled yet. Recruiter bookings will appear here with calendar sync status.
              </div>
            )}
          </motion.div>
        )}
        ...
        {activeTab === 'saves' && (
          ...
          >
            ...
            <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-6 shadow-sm flex flex-col gap-4">
              <div>
                <h4 className="text-sm font-bold text-slate-800 dark:text-white">Active Submissions ({appliedJobs.length})</h4>
              ...
              <div className="flex flex-col gap-3">
                {appliedJobs.length > 0 ? (
                  appliedJobs.map(job => (
                    <div key={job.id} className="p-4 rounded-2xl border border-slate-100 dark:border-navy-800 bg-slate-50/20 dark:bg-navy-900/20 flex justify-between items-center gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h5 className="text-xs font-bold text-slate-800 dark:text-white leading-tight">{job.title}</h5>
                          <p className="text-[9.5px] text-slate-500 mt-0.5">{job.company} • <span className="text-slate-500 font-bold">{job.salary}</span></p>
                        </div>
                      </div>
            ...
            <div className="border border-slate-200/60 dark:border-navy-800/40 bg-white/70 dark:bg-navy-900/40 rounded-3xl p-6 shadow-sm flex flex-col gap-4">
              <div>
                <h4 className="text-sm font-bold text-slate-800 dark:text-white">Saved Openings ({savedJobs.length})</h4>
